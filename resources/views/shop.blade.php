...
@if ($shop==null)
<div class="row mb-4">
 <h4 class="text-center">You Have no Shop. Do You Want to Open One?</h4>
    </div>
 <div class="position-relative">
    <a href="/newshop"class="btn position-absolute start-50 translate-middle btn-outline-primary mx-auto">Open New Shop</a>
    </div>
    @else
    <div class="row row mb-4 justify-content-between align-items-center"><h4 class="col col-md-auto">{{ $shop->name }}</h4>
     <div class="col col-md-auto fw-bold">Total Hasil Penjualan = Rp. {{ $revenue}},-</div>
    </div>
    ...
    <div class="col col-md-auto">Open From : {{ $shop->OpeningDay }} to {{ $shop->ClosingDay }} For 24 Hours</div>
        @else
    <div class="col col-md-auto">Open From : {{ $shop->OpeningDay }} to {{ $shop->ClosingDay }} At {{ $shop->OpeningHours }} to {{ $shop->ClosingHours }}</div>
        @endif


     <div class="col col-md-auto fw-bold">Total Item Terjual: {{ $selleditem }}</div>
    </div>
    <div class="row mb-4 justify-content-start"><a href="/insert" class="col col-sm-2 btn btn-primary">Add Product</a></div>
    ...
    <div class="row  mt-5">
        ...
        <div class="col col-md-auto">
            <a href="/update/{{ $item->id }}" class="card shadow rounded text-decoration-none text-dark"
                >
